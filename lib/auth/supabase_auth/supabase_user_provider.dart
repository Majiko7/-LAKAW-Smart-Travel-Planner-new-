...
class LakawSupabaseUser extends BaseAuthUser {
  LakawSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> lakawSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = LakawSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
